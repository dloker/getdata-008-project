...
loadActivityLabelsDataSet <- function() {
  ...
  activityNames <- read.table(file="./UCI HAR Dataset/activity_labels.txt", col.names=c("activityLabel", "activityName"))
...
getSimpleFitnessDataSet <- function(featureNameTable) {
  ...
  fitnessDataSet <- cbind(fitnessDataSet, "activity" = activityLabelDataSet$activityName, subjectDataSet)
  fitnessDataSet
}

# Use features.txt to get appropriate descriptive names for features.
featureNameTable <- read.table(file="./UCI HAR Dataset/features.txt", col.names = c("featureNumber", "featureName"))
# Filter table down to only those features of mean() and std()
featureNameTable <- featureNameTable[grepl("(mean\\(\\))|(std\\(\\))", featureNameTable$featureName),]
featureNameTable$featureName = featureNameTable$featureName[,drop = TRUE]

# Get a simplified fitness data set based on the features chosen, combining X, y (activity label), and subject
...
tidyAveragedFitnessDataSet <- fitnessDataset[,lapply(.SD,mean, na.rm=TRUE), by=list(subject,activity)]

write.table(x = tidyAveragedFitnessDataSet, file = "tidy-averageFitnessFeaturesBySubjectActivity.txt", row.name=FALSE)
